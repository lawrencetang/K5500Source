...
PTDetectionFlow::PTDetectionFlow(OSerialPort *serialPort, OSpectrograph *spectrograph, QObject *parent) :
    QObject(parent) {
...
PTDetectionFlow::~PTDetectionFlow() {

}

/**
 * @brief PTDetectionFlow::blank
 * @param setting
 * @return
 */
QString PTDetectionFlow::blank(PTSettingModel *setting) {
    m_ctBlankList.clear();
...
/**
 * @brief PTDetectionFlow::measure
 * @param setting
 * @return
 */
QString PTDetectionFlow::measure(PTSettingModel *setting) {
    ...
    if (setting->getDetectionMethod() == "Cuvette") {
        ...
        if (setting->getOptical() != "Auto") {
            ...
            ok = excuteMicroscaleMeasure(setting, setting->getOptical(),
                                         setting->getIdList(), NULL, PTDetectionFlow::showResult);
        } else {
            // 自动光程
            QList<int> stdId;
            QList<int> maxId;
            ...
            if (!stdId.isEmpty()) {
                ok = excuteMicroscaleMeasure(setting, setting->getMicroscaleStdOptical(),
                                             stdId, NULL, PTDetectionFlow::showResult);
                if (ok != "Success") {
                    return ok;
                }
            }
            ...
            if (!minId.isEmpty()) {
                ...
                ok = excuteMicroscaleMeasure(setting, setting->getMicroscaleMinOptical(),
                                             minId, NULL, PTDetectionFlow::showResult);
                if (ok != "Success") {
                    return ok;
                }
            }
            if (!maxId.isEmpty()) {
                ...
                ok = excuteMicroscaleMeasure(setting, setting->getMicroscaleMaxOptical(),
                                             maxId, NULL, PTDetectionFlow::showResult);
                if (ok != "Success") {
                    return ok;
                }
            }
...
/**
 * @brief PTDetectionFlow::transformUnit
 * @param unit
 * @return
 */
double PTDetectionFlow::transformUnit(QString unit) {
    if (unit == "ng/uL" ||
 ...
 * @param timeout
 */
void PTDetectionFlow::delay(int timeout) {
...
/**
 * @brief PTDetectionFlow::showResult
 * @param a
 */
void PTDetectionFlow::showResult(PTSettingModel *setting, Absorbance *a, int id, QString optical) {
...
/**
 * @brief PTDetectionFlow::prejudgement
 * @param setting
 * @param stdId
 * @param maxId
 * @param minId
 * @return
 */
QString PTDetectionFlow::prejudgement(PTSettingModel *setting,
...
/**
 * @brief PTDetectionFlow::excuteMicroscaleBlank
 * @param setting
 * @param opticalStr
 * @return
 */
QString PTDetectionFlow::excuteMicroscaleBlank(PTSettingModel *setting, QString opticalStr) {
...
/**
 * @brief PTDetectionFlow::excuteMicroscaleMeasure
 * @param setting
 * @param opticalStr
 * @param excuteId
 * @param result
 ...
 * @return
 */
QString PTDetectionFlow::excuteMicroscaleMeasure(PTSettingModel *setting, QString opticalStr,

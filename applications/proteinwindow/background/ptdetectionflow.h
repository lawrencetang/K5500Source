...
typedef void (PTDetectionFlow::*pShowResultFunc)(PTSettingModel *setting, Absorbance *a, int id, QString optical);

class PTDetectionFlow : public QObject
{
    Q_OBJECT
public:
    explicit PTDetectionFlow(OSerialPort *serialPort, OSpectrograph *spectrograph, QObject *parent = 0);
    ~PTDetectionFlow();

    QString blank(PTSettingModel *setting);
    QString measure(PTSettingModel *setting);
...
private:
    ...
    void showResult(PTSettingModel *setting, Absorbance *a, int id, QString optical);
    QString prejudgement(PTSettingModel *setting, QList<int> &stdId, QList<int> &maxId, QList<int> &minId);
    QString excuteMicroscaleBlank(PTSettingModel *setting, QString opticalStr);
    QString excuteMicroscaleMeasure(PTSettingModel *setting, QString opticalStr,

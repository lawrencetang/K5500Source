...
#define DATA_PATH "data/microarray/"

MATestingWidget::MATestingWidget(QWidget *parent) :
    QFrame(parent),
    ui(new Ui::MATestingWidget) {
    ui->setupUi(this);
...
MATestingWidget::~MATestingWidget() {
    delete ui;
}

/**
 * @brief MATestingWidget::setPlotTitle
 * @param title
 */
void MATestingWidget::setPlotTitle(QString title) {
    m_pPlot->setPlotTitle(m_plotTitle + "(" + title + ")");
}

/**
 * @brief MATestingWidget::setOperatingKeyEnable
 * @param ok
 */
void MATestingWidget::setOperatingKeyEnable(bool ok) {
    if (m_pDyMsgBox != NULL) {
        m_pDyMsgBox->close();
        delete m_pDyMsgBox;
        m_pDyMsgBox = NULL;
...
/**
 * @brief MATestingWidget::setAnimationEnable
 * @param ok
 */
void MATestingWidget::setAnimationEnable(bool ok) {
    m_pFileExplorer->setAnimationEnable(ok);
    m_pRecordTableWidget->setAnimationEnable(ok);
}

/**
 * @brief MATestingWidget::showEvent
 * @param event
 */
void MATestingWidget::showEvent(QShowEvent *event) {
    Q_UNUSED(event)
    if (!m_isShow) {
        m_isShow = true;
...
/**
 * @brief MATestingWidget::mousePressEvent
 * @param event
 */
void MATestingWidget::mousePressEvent(QMouseEvent *event) {
    m_isPressed = true;
    m_pressPos = event->pos();
}

/**
 * @brief MATestingWidget::mouseReleaseEvent
 * @param event
 */
void MATestingWidget::mouseReleaseEvent(QMouseEvent *event) {
    m_isPressed = false;
...
/**
 * @brief MATestingWidget::drawStaticCurve
 * @param curveName
 * @param x
 * @param y
 */
void MATestingWidget::drawStaticCurve(QString title, QVector<double> x, QVector<double> y) {
    m_pPlot->drawStaticCurve(title, x, y);
}

/**
 * @brief MATestingWidget::saveResult
 * @param fileName
 * @param data
 */
void MATestingWidget::saveResult(QString fileName, QStringList data) {
...
/**
 * @brief MATestingWidget::saveRecord
 * @param data
 */
void MATestingWidget::saveRecord(QStringList data) {
    m_pRecordTableWidget->insert(data);
}

/**
 * @brief MATestingWidget::capture
 */
void MATestingWidget::capture() {
...
/**
 * @brief MATestingWidget::showDyes
 * @param fileName
 */
void MATestingWidget::showDyes(QString fileName) {
    ui->frameHeader->setEnabled(false);
    QString path = QDir::currentPath() + "/" + DATA_PATH + "files/";
    fileName = path + fileName;
    QFile file(fileName);
    QList<QStringList> dyeInfos;
    if(file.open(QIODevice::ReadOnly)) {
        QTextStream in(&file);
        while(!in.atEnd()) {
            QString lineStr = in.readLine();
            if (lineStr.startsWith( "Dye:," )) {
                lineStr = lineStr.replace( "Dye:,", "" );
                QStringList dyeList = lineStr.split( ",", QString::SkipEmptyParts );
                if (!dyeList.isEmpty()) {
                    dyeInfos.append(dyeList);
                }
            }
        }
        file.close();
    }
    OTableViewUtil::showData(tr("Dye Information"), QStringList()<<tr("Dye Name") << tr("Wave Length") << tr("1/M*cm") << tr("260m%") << tr("280m%") << tr("Conc"), dyeInfos);
    ui->frameHeader->setEnabled(true);
}

/**
 * @brief MATestingWidget::showData
 * @param fileName
 */
void MATestingWidget::showData(QString fileName) {
...
/**
 * @brief MATestingWidget::doubleClickPlot
 */
void MATestingWidget::doubleClickPlot() {
...
/**
 * @brief MATestingWidget::doubleClickRecordTableWidget
 */
void MATestingWidget::doubleClickRecordTableWidget() {
...
/**
 * @brief MATestingWidget::onBlankBtnClicked
 */
void MATestingWidget::onBlankBtnClicked() {
    ui->frameHeader->setEnabled(false);
    m_pPlot->clearCurves();
...
/**
 * @brief MATestingWidget::onMeasureBtnClicked
 */
void MATestingWidget::onMeasureBtnClicked() {
    ui->frameHeader->setEnabled(false);
    m_pPlot->clearCurves();
...
/**
 * @brief MATestingWidget::initalizeSignals
 */
void MATestingWidget::initalizeSignals() {
    ...
    connect(m_pRecordTableWidget, SIGNAL(showItemDetail(QString)), this, SLOT(showDyes(QString)));
...
/**
 * @brief MATestingWidget::initalizeWidgets
 */
void MATestingWidget::initalizeWidgets() {

...
UvVisWindow::UvVisWindow(OSerialPort *serialPort, OSpectrograph *spectrograph, QWidget *parent) :
    QFrame(parent),
    ui(new Ui::UvVisWindow) {
    ui->setupUi(this);

    m_isBlanked = false;
    m_isAnimation = true;
    m_pDetectionFlow = new UVDetectionFlow(serialPort, spectrograph, this);
...
UvVisWindow::~UvVisWindow() {
    delete ui;
    delete m_pSettingModel;
}

/**
 * @brief UvVisWindow::back
 */
void UvVisWindow::back() {
    m_isBlanked = false;
...
/**
 * @brief UvVisWindow::showTestingPage
 */
void UvVisWindow::showTestingPage() {
...
/**
 * @brief UvVisWindow::showSettingPage
 */
void UvVisWindow::showSettingPage() {
...
/**
 * @brief UvVisWindow::startBlank
 */
void UvVisWindow::startBlank() {
...
/**
 * @brief UvVisWindow::startMeasure
 */
void UvVisWindow::startMeasure() {
    if (m_pDetectionFlow == NULL) {
        m_pTestingWidget->setOperatingKeyEnable(true);
        OMessageBoxUtil::staticNotice(0, tr( "System error, please restart to fix!" ), QStringList() << tr( "OK" ), 10);
        return;
...
/**
 * @brief UvVisWindow::closeEvent
 * @param e
 */
void UvVisWindow::closeEvent(QCloseEvent *e) {
    QFrame::closeEvent(e);
}

/**
 * @brief UvVisWindow::flipOverAnimation
 * @param w
 */
void UvVisWindow::flipOverAnimation(QWidget *w) {
...
/**
 * @brief UvVisWindow::initializeSignals
 */
void UvVisWindow::initializeSignals() {
...
/**
 * @brief UvVisWindow::initializeWidgets
 */
void UvVisWindow::initializeWidgets() {

    m_pSettingWidget = new UVSettingWidget();
    ...
    m_pSettingModel = new UVSettingModel();
    m_pSettingModel->setMaxChannelNumber(m_pSettingWidget->getMaxChannelNumber());
    m_pSettingModel->setMeasureParameters(m_pSettingWidget->getMeasureSetting());
    m_pSettingModel->setMotorParameters(m_pSettingWidget->getMotorSetting());

    m_pTestingWidget = new UVTestingWidget();

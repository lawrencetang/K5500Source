...
NADetectionFlow::NADetectionFlow(OSerialPort *serialPort, OSpectrograph *spectrograph, QObject *parent) :
    QObject(parent) {
...
NADetectionFlow::~NADetectionFlow() {

}

/**
 * @brief NADetectionFlow::blank
 * @param setting
 * @return
 */
QString NADetectionFlow::blank(NASettingModel *setting) {
    m_ctBlankList.clear();
...
/**
 * @brief NADetectionFlow::measure
 * @param setting
 * @return
 */
QString NADetectionFlow::measure(NASettingModel *setting) {
    ...
    if (setting->getDetectionMethod() == "Cuvette") {
        ...
        if (setting->getOptical() != "Auto") {
            ...
            ok = excuteMicroscaleMeasure(setting, setting->getOptical(),
                                         setting->getIdList(), NULL, NADetectionFlow::showResult);
        } else {
            // 自动光程
            QList<int> stdId;
            QList<int> maxId;
            ...
            if (!stdId.isEmpty()) {
                ok = excuteMicroscaleMeasure(setting, setting->getMicroscaleStdOptical(),
                                             stdId, NULL, NADetectionFlow::showResult);
                if (ok != "Success") {
                    return ok;
                }
            }
            ...
            if (!minId.isEmpty()) {
                ...
                ok = excuteMicroscaleMeasure(setting, setting->getMicroscaleMinOptical(),
                                             minId, NULL, NADetectionFlow::showResult);
                if (ok != "Success") {
                    return ok;
                }
            }
            if (!maxId.isEmpty()) {
                ...
                ok = excuteMicroscaleMeasure(setting, setting->getMicroscaleMaxOptical(),
                                             maxId, NULL, NADetectionFlow::showResult);
                if (ok != "Success") {
                    return ok;
                }
            }
...
/**
 * @brief NADetectionFlow::transformUnit
 * @param nuit
 * @return
 */
double NADetectionFlow::transformUnit(QString unit) {
    if (unit == "ng/uL" ||
 ...
 * @param timeout
 */
void NADetectionFlow::delay(int timeout) {
...
/**
 * @brief NADetectionFlow::showResult
 * @param a
 */
void NADetectionFlow::showResult(NASettingModel *setting, Absorbance *a, int id, QString optical) {

    QString fileName = QDateTime::currentDateTime().toString("yyyyMMddhhmmss") + ".csv";
    // 计算浓度,A230,A260,A280,标准单位为ng/uL
    double c = a->getAbsoByWL((double)setting->getMwl())*setting->getCoefficient()*transformUnit(setting->getUnit());
...
/**
 * @brief NADetectionFlow::prejudgement
 * @param setting
 * @param stdId
 * @param maxId
 * @param minId
 * @return
 */
QString NADetectionFlow::prejudgement(NASettingModel *setting,
...
/**
 * @brief NADetectionFlow::excuteMicroscaleBlank
 * @param setting
 * @param opticalStr
 * @return
 */
QString NADetectionFlow::excuteMicroscaleBlank(NASettingModel *setting, QString opticalStr) {
...
/**
 * @brief NADetectionFlow::excuteMicroscaleMeasure
 * @param setting
 * @param opticalStr
 * @param excuteId
 * @param result
 ...
 * @return
 */
QString NADetectionFlow::excuteMicroscaleMeasure(NASettingModel *setting, QString opticalStr,

...
NGDetectionFlow::NGDetectionFlow(OSerialPort *serialPort, OSpectrograph *spectrograph, QObject *parent) :
    QObject(parent) {
...
NGDetectionFlow::~NGDetectionFlow() {

}

/**
 * @brief NGDetectionFlow::blank
 * @param setting
 * @return
 */
QString NGDetectionFlow::blank(NGSettingModel *setting) {
    m_ctBlankList.clear();
...
/**
 * @brief NGDetectionFlow::measure
 * @param setting
 * @return
 */
QString NGDetectionFlow::measure(NGSettingModel *setting) {
    ...
    if (setting->getDetectionMethod() == "Cuvette") {
        ...
        if (setting->getOptical() != "Auto") {
            ...
            ok = excuteMicroscaleMeasure(setting, setting->getOptical(),
                                         setting->getIdList(), NULL, NGDetectionFlow::showResult);
        } else {
            // 自动光程
            QList<int> stdId;
            QList<int> maxId;
            ...
            if (!stdId.isEmpty()) {
                ok = excuteMicroscaleMeasure(setting, setting->getMicroscaleStdOptical(),
                                             stdId, NULL, NGDetectionFlow::showResult);
                if (ok != "Success") {
                    return ok;
                }
            }
            ...
            if (!minId.isEmpty()) {
                ...
                ok = excuteMicroscaleMeasure(setting, setting->getMicroscaleMinOptical(),
                                             minId, NULL, NGDetectionFlow::showResult);
                if (ok != "Success") {
                    return ok;
                }
            }
            if (!maxId.isEmpty()) {
                ...
                ok = excuteMicroscaleMeasure(setting, setting->getMicroscaleMaxOptical(),
                                             maxId, NULL, NGDetectionFlow::showResult);
                if (ok != "Success") {
                    return ok;
                }
            }
...
/**
 * @brief NGDetectionFlow::transformUnit
 * @param unit
 * @return
 */
double NGDetectionFlow::transformUnit(QString unit) {
    if (unit == "ng/uL" ||
 ...
 * @param timeout
 */
void NGDetectionFlow::delay(int timeout) {
...
/**
 * @brief NGDetectionFlow::showResult
 * @param a
 */
void NGDetectionFlow::showResult(NGSettingModel *setting, Absorbance *a, int id, QString optical) {
...
/**
 * @brief NGDetectionFlow::prejudgement
 * @param setting
 * @param stdId
 * @param maxId
 * @param minId
 * @return
 */
QString NGDetectionFlow::prejudgement(NGSettingModel *setting,
...
/**
 * @brief NGDetectionFlow::excuteMicroscaleBlank
 * @param setting
 * @param opticalStr
 * @return
 */
QString NGDetectionFlow::excuteMicroscaleBlank(NGSettingModel *setting, QString opticalStr) {
...
/**
 * @brief NGDetectionFlow::excuteMicroscaleMeasure
 * @param setting
 * @param opticalStr
 * @param excuteId
 * @param result
 ...
 * @return
 */
QString NGDetectionFlow::excuteMicroscaleMeasure(NGSettingModel *setting, QString opticalStr,
